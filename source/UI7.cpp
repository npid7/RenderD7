...
void UI7CtxPushDebugCmd(const DrawCmd& ref);
 ...
 public:
  ...
  void Process() {
    ...
    if (type == DrawCmdType_Rect) {
      ...
      RenderD7::R2::AddTriangle(R7Vec2(rect.x, rect.y), R7Vec2(rect.z, rect.w),
                                add_coords, clr);
...
void UI7DrawList::AddImage(R7Vec2 pos, RenderD7::Image& img) {
  DrawCmd cmd;
  cmd.screen = RenderD7::R2::GetCurrentScreen();
  cmd.rect.x = pos.x;
  cmd.rect.y = pos.y;
...
void UI7DrawList::AddCall(const DrawCmd& cmd) { this->list.push_back(cmd); }
...
void UI7DrawList::AddDebugCall(const DrawCmd& cmd) {
  DrawCmd dcmd;
...
struct UI7Menu {
  ...
  NTCtrl ctrl;                     // NonTouchControl

  // SubMenu
  std::string submenu;

  // DrawLists
  UI7DrawList* background;
  UI7DrawList* main;
  UI7DrawList* front;
...
struct UI7_Ctx {
  ...
  bool debugging;
  bool debug_menu;
  std::map<std::string, UI7Menu> menus;
  std::vector<UI7Menu*> active_menus;
  UI7DrawList debug_calls;
  UI7DrawList bdl;
  UI7DrawList fdl;
  UI7Menu* cm;
};
...
bool UI7CtxBeginMenu(const std::string& lb) {
  ...
  if (ui7_ctx.menus.find(id.ID()) == ui7_ctx.menus.end())
    ui7_ctx.menus.insert(std::make_pair(id.ID(), UI7Menu()));
  ...
  if (!ui7_ctx.cm->background) ui7_ctx.cm->background = new UI7DrawList();
  if (!ui7_ctx.cm->main) ui7_ctx.cm->main = new UI7DrawList();
  if (!ui7_ctx.cm->front) ui7_ctx.cm->front = new UI7DrawList();
...
void UI7CtxEndMenu() {
  ...
  if (ui7_ctx.cm->enable_scrolling) {
    ...
    if (ui7_ctx.cm->show_scroolbar) {
      ...
      int slider_pos =
          tsp +
          d7min(
              static_cast<float>(szs - slider_rh - 4),
              d7max(0.f, static_cast<float>(
                             (szs - slider_rh) *
  ...
  if (ui7_ctx.debug_menu) {
    ...
    str << "ui7_ctx.cm->cb: (" << ui7_ctx.cm->cb.x << ", " << ui7_ctx.cm->cb.y
        << ")" << std::endl;
    ...
    str << "ui7_ctx.cm->ms: (" << ui7_ctx.cm->ms.x << ", " << ui7_ctx.cm->ms.y
        << ")" << std::endl;
...
bool Button(const std::string& label, R7Vec2 size) {
  ...
  ui7_ctx.cm->main->AddText(pos, label, RenderD7::ThemeActive().AutoText(btn));
  return ret;
}
...
void Progressbar(float value) {
  ...
  if (ui7_ctx.cm->show_scroolbar && ui7_ctx.cm->enable_scrolling) size.x -= 16;
  MoveCursor(size);
  ui7_ctx.cm->ctrl.AddObj();
...
void ColorSelector(const std::string& label, unsigned int& color) {
  ...
  if (ui7_ctx.cm->submenu == id.ID()) {
    ...
    ui7_ctx.cm->front->AddText(
        ...
        RenderD7::ThemeActive().AutoText(RD7Color_FrameBg), RD7TextFlags_Short);
    // Add luminance text
    ui7_ctx.cm->front->AddText(
    ...
    // Red
    {
      ...
      ui7_ctx.cm->front->AddText(npos + R7Vec2(2, cbs.y * 3 + 4),
                                 "R: " + std::to_string(clr.m_r), RD7Color_Text,
                                 RD7TextFlags_AlignMid, R7Vec2(50, 0));
    }
    // Green
    {
      ...
      ui7_ctx.cm->front->AddText(npos + R7Vec2(54, cbs.y * 3 + 4),
    ...
    // Blue
    {
      ...
      ui7_ctx.cm->front->AddText(npos + R7Vec2(2, cbs.y * 4 + 4),
    ...
    // Alpha
    {
      ...
      ui7_ctx.cm->front->AddText(npos + R7Vec2(54, cbs.y * 4 + 4),
  ...
  ui7_ctx.cm->main->AddRectangle(pos + R7Vec2(2, 2), cbs - R7Vec2(4, 4), color);
  ui7_ctx.cm->main->AddText(
      pos + R7Vec2(cbs.x + 5, 1), label,
      RenderD7::ThemeActive().AutoText(RD7Color_Background));
}
...
UI7DrawList* Menu::GetBackgroundList() {
  if (!UI7CtxValidate()) return nullptr;
  if (!UI7CtxInMenu()) return &ui7_ctx.bdl;
  return ui7_ctx.cm->background;
}

UI7DrawList* Menu::GetList() {
  if (!UI7CtxValidate()) return nullptr;
  if (!UI7CtxInMenu()) return &ui7_ctx.bdl;
  return ui7_ctx.cm->main;
}

UI7DrawList* Menu::GetForegroundList() {
  if (!UI7CtxValidate()) return nullptr;
  if (!UI7CtxInMenu()) return &ui7_ctx.bdl;
